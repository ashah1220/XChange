...
public class GeminiStreamingMarketDataServiceTest {
    ...
    @InjectMocks
    GeminiStreamingMarketDataService geminiStreamingMarketDataService;
     ...
     * defined in the product subscription passed to GeminiStreamingMarketDataService.connect(productSubscription)
    ...
    public void getOrderBook_InvalidPair() {
        ...
        try {
            geminiStreamingMarketDataService
                    .getOrderBook(LTC_USD)
                    .subscribe(orderBook -> {
                    });
        } catch(Exception e) {
            System.out.println(e.getMessage());
            assertEquals(String.format("The currency pair %s is not subscribed for orderbook", LTC_USD),
                    e.getMessage());
            throw e;
        }
    }
